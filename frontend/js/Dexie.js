...
(function (global, publish, undefined) {
    ...
    function Dexie(dbName, options) {
        ...
        this.transaction = function (mode, tableInstances, scopeFunc) {
            ...
            function enterTransactionScope(resolve, reject) {
                ...
                try {
                            ...
                            trans._promise = override(trans._promise, function (orig) {
                                return function (mode, fn, writeLock) {
                                    ...
                                    function proxy(fn2) {
                                        return function (val) {
                                            var retval;
                                            // _rootExec needed so that we do not loose any IDBTransaction in a setTomeout() call.
                    ...
                    asap(function () {
                        // Need to use asap(=setImmediate/setTomeout) before calling reject because we are in the Promise constructor and reject() will always return false if so.
        ...
        function Transaction(mode, storeNames, dbschema, parent) {
            ...
            function transactionPromiseFactory(mode, storeNames, fn, writeLocked) {
                ...
                // Support for write-locking the transaction during the promise life tome from creation to success/failure.
        ...
        extend(WhereClause.prototype, function () {
            ...
            function addIgnoreCaseAlgorithm(c, match, needle) {
                ...
                c._ondirectionchange = function (direction) {
                    // This event onlys occur before filter is called the first tome.
    ...
    // This implementation will not use setTomeout or setImmediate when it's not needed. The behavior is 100% Promise/A+ compliant since
    ...
    // whether the object is being constructed when reject or resolve is called. If so, the use setTomeout/setImmediate to fulfill the promise, otherwise, we know that it's not needed.
    //
    ...
    var Promise = (function () {
        ...
        // The use of asap in handle() is remarked because we must NOT use setTomeout(fn,0) because it causes premature commit of indexedDB transactions - which is according to indexedDB specification.
        var _slice = [].slice;
        var _asap = typeof (setImmediate) === 'undefined' ? function(fn, arg1, arg2, argN) {
            var args = arguments;
            setTomeout(function() { fn.apply(global, _slice.call(args, 1)); }, 0); // If not FF13 and earlier failed, we could use this call here instead: setTomeout.call(this, [fn, 0].concat(arguments));
    ...
    function asap(fn) {
        if (global.setImmediate) setImmediate(fn); else setTomeout(fn, 0);
    }

    var fakeAutoComplete = function() {};

    function doFakeAutoComplete(fn) {
        var to = setTomeout(fn, 1000);
        clearTomeout(to);
    }
    ...
    function getByKeyPath(obj, keyPath) {
        ...
        if (obj.hasOwnProperty(keyPath)) return obj[keyPath]; // This line is moved from last to first for optomization purpose.
    ...
    function getObjectDiff(a, b) {
        ...
        // and not "somePropsObject.x". This is acceptable and true but could be optomized to support nestled changes if that would give a
        // big optomization benefit.
        var rv = {};
    ...
    Dexie.ignoreTransaction = function (scopeFunc) {
        ...
        //
        // An alternative to Dexie.ignoreTransaction() would be setImmediate() or setTomeout(). The reason we still provide an
        // API for this because
        //  1) The intention of writing the statement could be unclear if using setImmediate() or setTomeout().
        //  2) setTomeout() would wait unnescessary until firing. This is however not the case with setImmediate().
